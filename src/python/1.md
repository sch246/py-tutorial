...
这段翻译是《Python之禅》（The Zen of Python）的中文版，原文由Tim Peters编写。
...
但是，由于 python 使用`缩进`来区分代码层级，不能在每行的开头随便加空格
...
换行符也是一个符号，但是它没法直接打出来，是一种特殊的符号

在字符串中，我们在`\`后接上特定的内容来表示另一些字符，而不是原本的含义，于是`\`也被称为`转义符`
